...
| **[`fxstore`](https://github.com/fixpoint-linux/fxstore)** | A **content-addressed build store** — reads a Dhall package set, computes the dependency closure as a least fixed point with `datalog-dafsa`, and builds each package's typed recipe into `/fx/store/<hash>-<name>`. The distro's storage + build layer. |
